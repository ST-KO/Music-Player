...
// This obj will contain the songs, the current song playing, the time of the current song
let userData = {
    
    // To allow users to shuffle and delete songs from playlist without mutating the original array
    songs: [...allSongs],
    currentSong: null,
    songCurrentTime: 0
}; 
...
const playSong = (id) =>{
    ...
    // 'if' statement is to make sure the song starts from the beginning when play
    ...
    // No need to use optional chaining operator '?' here since 'userData.currentSong' will not be 'null' or 'undefined'
...
const shuffle = () => {
    ...
    // To randomise items in songs array
...
const deleteSong = (id) => {
    ...
    // Resetting 'userData' object to its original state if playlist is empty
...
// To display the songs in the UI
...
// To play next song automatically when the current playing song ends
